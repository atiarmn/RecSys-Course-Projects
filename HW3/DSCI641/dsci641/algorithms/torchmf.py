...
class MFBatch(NamedTuple):
    ...
    def to(self, dev: str) -> MFBatch:
        "move this batch to a device"
        return MFBatch(self.users.to(dev), self.items.to(dev), self.rates.to(dev))
...
class MFEpochData:
    ...
    def batch(self, num) -> MFBatch:
        ...
        rows = torch.from_numpy(self.permutation[start:end])
        if rows.device != self.t_items.device:
            rows = rows.to(self.t_items.device)
...
class MFTrainData:
    ...
    def for_epoch(self, rng: np.random.Generator, device=None) -> MFEpochData:
...
class TorchMF(Predictor):
    ...
    def fit(self, ratings, device=None, **kwargs):
        # run the iterations
    ...
    def _fit_iter(self):
        ...
        epoch_data = self._data.for_epoch(self._rng, self._current_device)
        # set up a progress bar
        loop = tqdm(range(epoch_data.batch_count))
        for i in loop:
            # create input tensors from the data
            uv, iv, rv = epoch_data.batch(i)
